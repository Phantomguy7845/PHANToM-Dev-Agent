...
const fetch = require('node-fetch'); // ใช้สำหรับเรียก OpenAI API
...
// ---------- IPC: AI Chat (OpenAI provider v0) ----------

ipcMain.handle('ai:chat', async (event, { providerId, apiKey, messages, options }) => {
  if (providerId !== 'openai') {
    throw new Error('ยังรองรับเฉพาะ OpenAI เท่านั้น');
  }
  if (!apiKey) {
    throw new Error('ยังไม่ได้ใส่ API key');
  }

  const model = options?.model || 'gpt-4.1-mini'; // เปลี่ยนตามที่คุณใช้จริง
